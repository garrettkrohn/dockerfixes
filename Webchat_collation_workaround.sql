...
proc:BEGIN
    ...
    SET @procedureName := 'createWebChat' COLLATE utf8mb4_0900_ai_ci;
    SET @tablesAffected := 'location, stored_message_categories, stored_message' COLLATE utf8mb4_0900_ai_ci;
    SET @variablesProvided := CONCAT('company_id: ', companyId, 'location_name: ', locationName, 'phone_number_id: ', phoneNumberId,
    'style_class: ', styleClass, 's3_style_file: ', s3StyleFile, 'created_by_user_id: ', createdByUserId, 'hide_mobile: ', hideMobile) COLLATE utf8mb4_0900_ai_ci;
    SET @results := '' COLLATE utf8mb4_0900_ai_ci;
    SET @rowsAffected := 0 COLLATE utf8mb4_0900_ai_ci;

    -- Define variables for procedure
    SET @widgetName = 'Online Chat' COLLATE utf8mb4_0900_ai_ci;
    SET @locationType = 'std' COLLATE utf8mb4_0900_ai_ci;
    SET @categoryName = locationName COLLATE utf8mb4_0900_ai_ci;
    SET @webWelcomeLabel = 'Web Welcome' COLLATE utf8mb4_0900_ai_ci;
    SET @webWelcomeText = 'Hello! How may we assist you?' COLLATE utf8mb4_0900_ai_ci;
    SET @mobileTransferLabel = 'Mobile Transfer' COLLATE utf8mb4_0900_ai_ci;
    SET @mobileTransferText = 'Thanks for chatting with us today. To continue your conversation via text, just reply to this message on your phone.'COLLATE utf8mb4_0900_ai_ci;
    SET @afterHoursWelcomeLabel = 'After Hours Welcome'COLLATE utf8mb4_0900_ai_ci;
    SET @afterHoursWelcomeText = 'Thanks for chatting with us today. We are currently closed. Please leave us your mobile number and a message and we will get back to you as soon as we can.'COLLATE utf8mb4_0900_ai_ci;
    SET @afterHoursMobileLabel = 'After Hours Mobile Transfer'COLLATE utf8mb4_0900_ai_ci;
    SET @afterHoursMobileText = 'Thanks for chatting with us today. We will send you a text as soon as our team is available.'COLLATE utf8mb4_0900_ai_ci;
    SET @widgetDeviceId = 'main-chat-page'COLLATE utf8mb4_0900_ai_ci;
    SET @widgetSession = 'session'COLLATE utf8mb4_0900_ai_ci;
    SET @realWidgetDeviceId = ( SELECT CONCAT(@widgetDeviceId, COUNT(*) + 1) FROM `widget` WHERE `company_id` = companyId AND `device_id` LIKE CONCAT(@widgetDeviceId, '%'))COLLATE utf8mb4_0900_ai_ci;
    SET @storedMessageType = 'live_web_chat'COLLATE utf8mb4_0900_ai_ci;
    SET @storedMessageCategoryType = 'live_web_chat'COLLATE utf8mb4_0900_ai_ci;

    -- Validate parameters given

    -- Check companyId to ensure it isn't empty
    IF (companyId = '') THEN 
        SELECT ('Please provide a valid company_id') COLLATE utf8mb4_0900_ai_ci; 
        LEAVE proc; 
    END IF; 
    
     -- Check locationName to ensure it isn't empty
    IF (locationName = '') THEN 
        SELECT ('Please provide a valid location_name')COLLATE utf8mb4_0900_ai_ci; 
        LEAVE proc; 
    END IF; 

     -- Check styleClass to ensure it isn't empty
    IF (styleClass = '') THEN 
        SELECT ('Please provide a valid style_class')COLLATE utf8mb4_0900_ai_ci; 
        LEAVE proc; 
    END IF; 

     -- Check s3StyleFile to ensure it isn't empty
    IF (s3StyleFile = '') THEN 
        SELECT ('Please provide a valid s3_style_file')COLLATE utf8mb4_0900_ai_ci; 
        LEAVE proc; 
    END IF; 
    
     -- Check createdByUserId to ensure it isn't empty
    IF (createdByUserId = '') THEN 
        SELECT ('Please provide a valid user_id')COLLATE utf8mb4_0900_ai_ci; 
        LEAVE proc; 
    END IF; 
    ...
    IF NOT EXISTS(SELECT 1 FROM `company` WHERE `company_id` = companyId) THEN
        SELECT CONCAT('company_id "', companyId, '" not found! Please provide a valid company_id')COLLATE utf8mb4_0900_ai_ci;
        LEAVE proc; 
    END IF;
    ...
    IF (phoneNumberId IS NOT NULL) THEN
        IF NOT EXISTS(SELECT 1 FROM `number` WHERE `phone_number_id` = phoneNumberId) THEN
            SELECT CONCAT('phone_number_id "', phoneNumberId, '" not found! Please provide a valid phone_number_id')COLLATE utf8mb4_0900_ai_ci;
            LEAVE proc;
        END IF;
    END IF; 
    ...
    IF ((phoneNumberId IS NULL) AND (hideMobile = 0)) THEN
        ...
        Please choose valid phone_number_id if you want mobile button avaiable.')COLLATE utf8mb4_0900_ai_ci;
        LEAVE PROC;
    END IF;
    ...
    IF NOT EXISTS(SELECT 1 FROM `user` WHERE `id` = createdByUserId) THEN
        SELECT CONCAT('user_id "', createdByUserId, '" not found! Please provide a valid user_id')COLLATE utf8mb4_0900_ai_ci;
        LEAVE proc; 
    END IF; 
    ...
    IF (SELECT `value` FROM `company_settings` WHERE `key` = 'api_key_v1' AND `company_id` = companyId) IS NULL THEN
        ...
        SET @new_api_key = (SELECT (SHA1(CONCAT(now(), FLOOR(100000000 + RAND() * (1000000000 - 100000000)))))) COLLATE utf8mb4_0900_ai_ci;
    ...
    SET @locationId = (SELECT `loc_id` FROM `location` WHERE `company_id` = companyId AND `name` = locationName) COLLATE utf8mb4_0900_ai_ci;

    -- Make sure that the stored messages have been created
    SET @realCategoryName = ( SELECT CONCAT(@categoryName, COUNT(*) + 1) FROM `stored_message_categories`  WHERE `company_id` = companyId AND `name` LIKE CONCAT(@categoryName, '%') COLLATE utf8mb4_0900_ai_ci);
    ...
    SET @categoryId = (SELECT `id` FROM `stored_message_categories` WHERE `company_id` = companyId AND `parent` = 0 AND `type` = @storedMessageCategoryType AND `name` = @realCategoryName) COLLATE utf8mb4_0900_ai_ci;
    SET @rowsAffected := @rowsAffected + ROW_COUNT() COLLATE utf8mb4_0900_ai_ci;
    ...
    SET @welcomeStoredId = (SELECT `stored_id` FROM `stored_message` WHERE `company_id` = companyId AND `type` = @storedMessageType AND `text` = @webWelcomeText AND `label` = @webWelcomeLabel AND `category` = @categoryId AND `order` = 1) COLLATE utf8mb4_0900_ai_ci;
    SET @rowsAffected := @rowsAffected + ROW_COUNT() COLLATE utf8mb4_0900_ai_ci;

    INSERT INTO `stored_message` (`company_id`, `type`, `text`, `label`, `category`, `order`)
        VALUES( companyId, @storedMessageType, @mobileTransferText, @mobileTransferLabel, @categoryId, 2);
    SET @mobileStoredId = (SELECT `stored_id` FROM `stored_message` WHERE `company_id` = companyId AND `type` = @storedMessageType AND `text` = @mobileTransferText AND `label` = @mobileTransferLabel AND `category` = @categoryId AND `order` = 2) COLLATE utf8mb4_0900_ai_ci;
    SET @rowsAffected := @rowsAffected + ROW_COUNT()COLLATE utf8mb4_0900_ai_ci;

    INSERT INTO `stored_message` (`company_id`, `type`, `text`, `label`, `category`, `order`)
        VALUES( companyId, @storedMessageType, @afterHoursWelcomeText, @afterHoursWelcomeLabel, @categoryId, 3);
    SET @afterHoursWelcomeStoredId = (SELECT `stored_id` FROM `stored_message` WHERE `company_id` = companyId AND `type` = @storedMessageType AND `text` = @afterHoursWelcomeText AND `label` = @afterHoursWelcomeLabel AND `category` = @categoryId AND `order` = 3)COLLATE utf8mb4_0900_ai_ci;
    SET @rowsAffected := @rowsAffected + ROW_COUNT()COLLATE utf8mb4_0900_ai_ci;

    INSERT INTO `stored_message` (`company_id`, `type`, `text`, `label`, `category`, `order`)
        VALUES( companyId, @storedMessageType, @afterHoursMobileText, @afterHoursMobileLabel, @categoryId, 4);
    SET @afterHoursMobileStoredId = (SELECT `stored_id` FROM `stored_message` WHERE `company_id` = companyId AND `type` = @storedMessageType AND `text` = @afterHoursMobileText AND `label` = @afterHoursMobileLabel AND `category` = @categoryId AND `order` = 4)COLLATE utf8mb4_0900_ai_ci;
    SET @rowsAffected := @rowsAffected + ROW_COUNT()COLLATE utf8mb4_0900_ai_ci;
    ...
    LIMIT 1;
    SET @widgetId = LAST_INSERT_ID() COLLATE utf8mb4_0900_ai_ci;
    SET @rowsAffected := @rowsAffected + ROW_COUNT() COLLATE utf8mb4_0900_ai_ci;
